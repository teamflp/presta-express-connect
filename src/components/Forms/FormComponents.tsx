...
export const FormInput: React.FC<InputProps> = ({
  ...
  return <div className="mb-5">
      ...
      <div className="relative group">
        {icon && <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
            {icon}
          </div>}
        <input id={id} type={inputType} className={`w-full px-4 ${icon ? 'pl-11' : 'pl-4'} py-3 rounded-lg border border-gray-300 
          shadow-sm transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-opacity-50
          bg-white ${className}`} {...props} />
        {type === 'password' && <button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? "Cacher le mot de passe" : "Afficher le mot de passe"} className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-gray-500 hover:text-gray-700 transition-colors rounded-none">
...
export const FormCheckbox: React.FC<CheckboxProps> = ({
  ...
  return <div className="flex items-center">
      <div className="relative inline-flex items-center">
        <input id={id} type="checkbox" className={`appearance-none w-5 h-5 rounded border border-gray-300 
          checked:bg-primary checked:border-primary 
          focus:outline-none focus:ring-2 focus:ring-offset-0 focus:ring-primary/30
          transition-all duration-200 ${className}`} {...props} />
        <svg className="absolute w-3.5 h-3.5 left-[3px] top-[4px] pointer-events-none opacity-0 peer-checked:opacity-100 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
...
export const FormButton: React.FC<ButtonProps> = ({
  ...
  return <button className={`${baseClasses} ${variantClasses[variant]} ${fullWidth ? 'w-full' : ''} ${isLoading ? 'opacity-80 cursor-not-allowed' : ''} ${className}`} disabled={isLoading || props.disabled} {...props}>
...
export const FormCard: React.FC<FormCardProps> = ({
  ...
  return <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-xl shadow-xl border border-gray-100 transition-all">
      <div className="text-center space-y-2">
        {typeof title === 'string' ? <h1 className="text-2xl font-bold text-gray-800">{title}</h1> : <h1 className="text-2xl font-bold text-gray-800">{title}</h1>}

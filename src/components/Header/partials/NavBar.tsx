...
const NavbarComponent = () => {
  ...
  return (
    <>
      <Navbar
        expand="lg"
        className={`navbar sticky-top ${isNavbarScroll ? 'navbar-scroll shadow-lg' : ''} ${location.pathname === '/' && !isNavbarScroll ? 'navbar-home' : ''}`}
      >
        <Container>
          {/* Brand */}
          ...
          <Navbar.Toggle
            ...
            className="navbar-toggler-custom d-lg-none border-0"
          >
            <Menu size={24} />
          </Navbar.Toggle>
              ...
              {isAuthenticated ? (
                <div className="d-flex align-items-center">
                  <Nav.Link 
                    ...
                    to={user?.userType === 'professional' ? "/professional-dashboard" : "/user-dashboard"}
                  >
                    <User size={16} />
                    {user?.userType === 'professional' ? 'Espace Pro' : 'Mon Compte'}
                  </Nav.Link>
                  <Nav.Link className="nav-link btn-outline-primary ms-2 py-2 px-3 rounded-3" onClick={logoutUser} style={{ cursor: 'pointer' }}>
                    Déconnexion
                  </Nav.Link>
                </div>
              ) : (
                <div className="d-flex align-items-center">
                  ...
                  <Nav.Link as={NavLink} className="btn btn-primary ms-2 py-2 px-3 rounded-3" to="/professional-login">
                    Espace Pro
                  </Nav.Link>
                </div>
              )}
            ...
            <div className="my-4">
              {isAuthenticated ? (
                <>
                  ...
                  <Nav.Link onClick={() => { logoutUser(); handleOffcanvasClose(); }} className="btn btn-outline-primary w-100 mt-3 d-flex justify-content-center" style={{ cursor: 'pointer' }}>
                    Déconnexion
                  </Nav.Link>
                </>
              ) : (

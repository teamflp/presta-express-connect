  ...
  useEffect(() => {
    ...
    const fetchProfessionals = async () => {
      try {
        ...
        const mockProfessionals = products.slice(0, 6).map(product => ({
          ...product,
          rating: (Math.random() * 2 + 3).toFixed(1), // Note entre 3 et 5
          speciality: job?.name || "Spécialiste",
          experience: Math.floor(Math.random() * 15) + 2 // Entre 2 et 17 ans d'expérience
        }));
                ...
                <div className="card h-100 shadow-sm border-0 rounded-3 overflow-hidden">
                  <div className="card-body p-4">
                    <div className="d-flex justify-content-between align-items-start mb-3">
                      <h5 className="card-title mb-0 fw-bold">{professional.name}</h5>
                      <div className="badge bg-primary d-flex align-items-center gap-1 px-2 py-1">
                        <Star size={14} fill="white" className="me-1" /> {professional.rating}
                      </div>
                    </div>
                    
                    <p className="text-muted mb-1">{professional.speciality}</p>
                    <p className="text-muted small mb-3">{professional.experience} ans d'expérience</p>
                    ...
                    <p className="card-text mb-4">{professional.descriptif.substring(0, 100)}...</p>
                    
                    <div className="d-grid gap-2">
                      <button className="btn btn-primary" style={{backgroundColor: '#C63E46', borderColor: '#C63E46'}}>
                        Contacter
                      </button>
